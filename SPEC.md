...
Every tier credits exactly its nominal chip amount on every purchase — there is no first-purchase bonus. The new-player cushion is provided by the one-time second-chance bonus (see the Chip Economy section), which is the single bonus pathway in V1.
...
4. Chip Shop — IAP store (five tiers from $0.99 to $19.99) with Restore Purchases affordance
...
| Session 16 | First-purchase doubler (per-install) | ❌ removed 2026-06-08 (see entry below) |
...
| 2026-05-25 | Play-bet decrement-at-placement now matches Ante / Blind / Trips. Reconciles the Ante/Blind-vs-Play decrement inconsistency surfaced after Session 30: Ante and Blind visibly left the stack at deal time, but the Play raise didn't leave the visible stack until the dealer-reveal beat — same situation (a wager committed to the felt), two behaviors. A player raising 4× on a big Ante was being shown a wrong number about what's already at risk right when they're sizing their next decision. All three Play bets (pre-flop 3×/4×, post-flop 2×, post-river 1×) now decrement `displayedBalance` at the wager via a one-line write in each VM wrapper. Engine math unchanged. Sharpens (does not regress) the Session 30 contract: what freezes during the dealer reveal is the *resolution credit*, not the wager. See the Architectural Decisions entry above for the layered-write mechanics and Session 31 in `HANDOFF.md` for the on-device gate. | ✅ in build [Session 31] — needs on-device pass per Session 26/28 lesson |
| 2026-06-08 | First-purchase 2× chip doubler removed entirely (reverses the Session 16 decision above). The one-time 2,500-chip second-chance bonus already covers the "give new players a cushion" intent, and the doubler at the $19.99 Deep Stack tier yielded 1.5MM chips on a first purchase — an outlier that distorted the chip economy and undercut the repeat-purchase incentive. A single bonus pathway (second-chance only) is cleaner. Every tier now credits exactly its nominal amount on every purchase. Removed: the `hasMadeFirstPurchase` persistence flag + its key, the 2× credit math in `ChipPurchaseProcessor`, the Chip Shop doubler banner + strikethrough, the `isFirstPurchase` plumbing through `PurchaseResult` / `ChipPurchaseProcessor.Outcome` / `TelemetryService.purchaseSucceeded`, and the doubler-specific telemetry params (`doubler_fired` / `doublered_chip_amount` / `is_first_purchase`); `iap.purchase.succeeded` now carries `product_id` + `chip_amount` only. | ✅ in build [Session 37] |
...
* **Play stake leaves the visible stack at the moment of the wager — same as Ante / Blind / Trips.** The three VM Play-bet wrappers (`betPreFlop`, `betPostFlop`, `betPostRiver`) explicitly `displayedBalance -= playBet` immediately after `dispatch(...)` and before `runAnimation`, gated on `errorMessage == nil` so a failed dispatch never leaks a phantom debit. This sharpens — does not contradict — the Session 30 contract. Session 30 made `syncFromGame` suppress all `displayedBalance` writes on the `.handComplete` transition so the post-resolution balance couldn't leak before the dealer reveal. The Session 30 test pinned that as "displayedBalance freezes entirely" but the intent was always narrower: what must not leak is the *resolution credit* (stake-back + net), not the player's just-committed wager. Engine-side, all four zones (Ante, Blind, Trips, Play) already debit `chipBalance` at placement via the same `chipStore.chipBalance -= required` mechanism ([SixthSeat/Sources/SixthSeat/GameState.swift:112](SixthSeat/Sources/SixthSeat/GameState.swift:112) / [:126](SixthSeat/Sources/SixthSeat/GameState.swift:126) / [:157](SixthSeat/Sources/SixthSeat/GameState.swift:157) / [:175](SixthSeat/Sources/SixthSeat/GameState.swift:175) / [:193](SixthSeat/Sources/SixthSeat/GameState.swift:193)); the Play case differed only at the display layer because every Play raise also calls `resolve()` in the same synchronous dispatch and therefore lands `.handComplete`, triggering the Session 30 gate. The VM layered write produces a post-debit intermediate value that lives on `displayedBalance` from the bet-wrapper line until the chip-resolution finalizer in `animateChipResolution` lands the engine-final value. Highest-risk cell — terminal post-river raise where placement and resolution share one dispatch — verified: the tap-debit (visible at the wrapper line) and the reveal-credit (landed by `animateChipResolution`) are now distinctly separated in time. Skip-to-settled, bypass-animation, and insufficient-chips paths all reconcile correctly: skip lands on engine-final via `finalizeSettledState`'s `pendingFinalBalance ?? chipBalance`; bypass collapses to engine-final on the same frame; insufficient-chips short-circuits at the `errorMessage == nil` guard before the subtract, so no phantom debit. Pinned by `playStakeLeavesStackAtTapResolutionWaitsForReveal` (rewrite of Session 30's test in the same slot), plus three multiplier-specific tap-debit tests, plus three forced-deck outcome-math tests (`playerFlushOnRiver`, `dealerDoesNotQualify`, `push`) that verify no double-count or dropped credit, plus a fold regression confirming the fix is surgical to Play raises. The Session 30 on-device gate (win / loss / push) extends to Session 31 with the additional check that Play chips visibly leave the stack at the tap on each Play raise — including the terminal post-river case. (Session 31)
* Post-hand flow: two buttons (REBET / NEW HAND). Confirmed Session 32, logged Session 33.

  - NEW HAND: dispatches .collectAndReset (all engine bets -> 0, phase -> .awaitingBets),
    then stages Ante = tableConfig.minimumAnte, Trips = 0. Does NOT deal. Player lands at
    .awaitingBets with table-min Ante/Blind pre-staged (Session 30 Fix 1 behavior) and can
    re-cycle Ante/Trips before dealing. Ante/Blind zones show the minimum via display
    fall-through: anteBet == 0 -> display falls to stagedAnte; Blind mirrors stagedAnte for
    the same reason. (Implementation: GameTableViewModel.newHand().)

  - REBET: guarded on canRebet (chipBalance >= lastAnte * 2). Stages Ante = lastAnte,
    Trips = lastTripsBet if affordable after the doubled Ante else 0, then calls deal()
    immediately (dispatched on the next runloop tick when animating, so SwiftUI renders the
    cleared state first) -- advancing to .preFlopDecision with the prior hand's wagers
    committed. Blind auto-matches Ante in-engine. PLAY is not posted until a pre-flop
    decision. (Implementation: GameTableViewModel.rebet().)

  Rationale: standard casino convention -- REBET is "repeat my stake," NEW HAND is "start
  fresh and re-decide." On-thesis for rehearsing real UTH rhythm. Discovered as undocumented
  build behavior in Session 32; verified correct via trace and adopted/logged rather than
  relitigated. Same pattern as the Session 22 clear-to-zero reversal: a found behavior that
  was verified and recorded, not a fresh decision.

  Implicit-behavior note for future sessions: the NEW HAND Ante/Blind display depends on the
