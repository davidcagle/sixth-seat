...
* **Bet-zone chip decomposition lives in the engine.** `ChipDecomposition` (`SixthSeat/Sources/SixthSeat/ChipDecomposition.swift`) is a public engine struct with a `bestFit(for amount: Int) -> ChipDecomposition?` helper that returns `(denomination, count)` — the largest available chip denomination ≤ amount and `amount / denomination` (integer division). Lives engine-side for the same reason as `StackHeight.bestFit` — pure integer arithmetic, asserted directly by engine tests. The bet zone reads the result and feeds `ChipStackView`, which routes `count` through `StackHeight.bestFit(for:)` to pick a stack-height variant. **Single-stack approximation by design** — off-grid bet amounts (e.g. $35 → ($25, 1)) accept a visual approximation because the precise dollar value is always rendered on the adjacent label. `bestFit` returns `nil` for `amount ≤ 0` and for amounts below the smallest chip ($5) so the bet zone naturally renders no chip stack when there's no wager. Available denominations are `[1000, 500, 100, 25, 5]` (largest first), matching the V1 chip set. (Session 21)
