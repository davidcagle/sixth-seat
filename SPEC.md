...
* **Bet-zone chip decomposition lives in the engine.** `ChipDecomposition` (`SixthSeat/Sources/SixthSeat/ChipDecomposition.swift`) is a public engine struct with a `bestFit(for amount: Int) -> ChipDecomposition?` helper that returns `(denomination, count)`. Lives engine-side for the same reason as `StackHeight.bestFit` — pure integer arithmetic, asserted directly by engine tests. The bet zone reads the result and feeds `ChipStackView`, which routes `count` through `StackHeight.bestFit(for:)` to pick a stack-height variant. **Largest clean-divisor multi-chip decomposition (Session 22 update).** `bestFit` picks the largest denomination D where `amount % D == 0` and returns `(D, amount / D)`, so the rendered stack is an exact representation of the bet: `$50 → (25, 2)` / `$75 → (25, 3)` / `$35 → (5, 7)` / `$1500 → (500, 3)`. The Session 21 version returned a lossy single-chip approximation (`$50 → (25, 1)`, visually identical to a `$25` bet); Session 22 replaced it with the exact-decomposition algorithm so `$25` and `$50` render as distinct stacks. `bestFit` returns `nil` for `amount ≤ 0` and for amounts that don't divide cleanly into any V1 denomination (e.g. `$1`, which the engine never produces — chip cycles step in multiples of `$5`). Available denominations are `[1000, 500, 100, 25, 5]` (largest first), matching the V1 chip set. (Session 21; algorithm updated Session 22)
